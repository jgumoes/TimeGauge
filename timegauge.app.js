...
var times = {
	...
	makeStrObj: function(i){
		var iTime = new Date(this.list[i].t);
		var ts = padTime(iTime.getHours())+":"+padTime(iTime.getMinutes())+":"+padTime(iTime.getSeconds());
		var ds = "";
		var prevTime;
		if(i > 0){
...
function devTimes(length){
	...
	var diffArr = new Array(length);
	//console.log(`is DST? ${(new Date()).getIsDST()}`);
	var milliseconds = Math.round(Date.now());
	for(i = 0; i < length-1; i++){
		let milli = Math.round(Math.random()*10*60 + 5*60)*1000;
		milliseconds -= milli;
		diffArr[i] = milli;
	}
...
function drawScroller(){
	var timeStrings = times.makeStrings();
...
function drawDiff(){
	...
	g.setFont("Vector", 20);
	g.setFontAlign(0,0);
	var diffStr = times.makeStrObj(0).d||"0s";
...
function updateClock() {
  ...
  var h = d.getHours(), m = d.getMinutes();
	var time = padTime(h)+":"+padTime(m)+ ":"+padTime(d.getSeconds());
